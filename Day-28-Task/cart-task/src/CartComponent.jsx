...
function CartComponent({showModal, toggle}) {
      ...
      showModal && (
        <div className="flex-col flex items-center fixed inset-0 left-1/4 bg-white dark:bg-black gap-8  p-10  text-black dark:text-white font-normal uppercase text-sm">
          ...
          </div>
          {
            ...
            <div className="flex flex-col ">
            <br/>
            <h3  className="text-lg"> Sub Total: ${getCartTotal()}</h3>
            <h3  className="text-lg">Shipping : Free</h3>
            <br/>
            <h1 className="text-lg font-bold">Total: ${getCartTotal()}</h1><br/>
            <button
              className="px-4 py-2 bg-gray-800 text-white text-xs font-bold uppercase rounded hover:bg-gray-700 focus:outline-none focus:bg-gray-700"
              onClick={() => {
                clearCart()
